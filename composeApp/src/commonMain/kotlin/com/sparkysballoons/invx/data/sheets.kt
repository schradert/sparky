...
import io.ktor.client.plugins.auth.BearerTokens
import io.ktor.client.plugins.auth.bearer
import io.ktor.client.plugins.auth.loadTokens
import io.ktor.client.plugins.contentnegotiation.ContentNegotiation
import io.ktor.client.plugins.logging.Logging
import io.ktor.client.plugins.logging.LogLevel
import io.ktor.client.request.get
import io.ktor.http.ClientRequestException
import io.ktor.http.HttpClient
...
class SheetsInventoryApi(private val googleOauthRepository: GoogleAuthRepository) : InventoryApi {
    ...
    private fun mapThrowable(t: Throwable) = when (t) {
        ...
        else -> ApiError(t.message)
    }

    private val runCatch = runMapCatch(::mapThrowable)

    override suspend fun getProducts(): DomainResult<List<Product>> = runCatch {
        val rows = client.get(url).body().values
        rows.drop(1).mapNotNull { parseProductRow(it, rows[0].map { it.lowercase() }) }                
    }

    override suspend fun getProductById(id: String): DomainResult<Product> = binding {
        getProducts()
            .bind()
